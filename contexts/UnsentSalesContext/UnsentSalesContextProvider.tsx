...
const UnsentSalesContextProvider = ({
  ...
  const removeFromUnsentSales = useCallback(
    (orderID: number) => {
      setUnsentSales((sales) =>
        sales.filter((sale) => sale.orderID !== orderID)
      );
    },
    [setUnsentSales]
  );
  ...
  const unsentSalesContext = useMemo(
    () => ({
      ...
      removeFromUnsentSales: removeFromUnsentSales,
    }),
    [unsentSales, addToUnsentSales, clearUnsentSales, removeFromUnsentSales]
  );
